...
function updateVerdict(data) {
  if (data.id === -1) {
    alert("Interviewer is NOT Available At This Time");
  } else if (data.id === -2) {
    alert("Interviewee is NOT Available At This Time");
  } else {
    location.reload();
  }
}
...
submitButton.onclick = function () {
  ...
  if (email1 === email2) {
    alert("Interviewer & Interviewee CANNOT Be The Same Person.");
    return;
  }
...
function insertRowIntoInterviewTable(data) {
  if (data.id === -1) {
    alert("Interviewer is NOT Available At This Tim");
    return;
  }
  if (data.id === -2) {
    alert("Interviewee is NOT Available At This Tim");
    return;
  }
  const table = document.querySelector("table tbody");
