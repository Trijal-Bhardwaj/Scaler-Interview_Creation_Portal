...
const mailService = require("./eMailSendingService");
let instance = null;
...
connection.connect((err) => {
  ...
  console.log("InterviewBit DB Table " + connection.state);
});
...
class DbService {
  ...
  // Table for Scheduled Interviews Loading
  ...
  async checkAvailability(email, start, end, id = -1) {
    try {
      ...
      const check2 = await new Promise((resolve, reject) => {
        const query =
          "SELECT (SELECT COUNT(*) FROM interviews WHERE email2 = ? and id != ?) - \
  ...
  async insertInterview(email1, email2, startTime, endTime) {
    try {
      ...
      if (check1 > 0) {
        console.log("Interviewer is NOT Available At This Time");
        return {
          id: -1,
        };
      } else if (check2 > 0) {
        console.log("Interviewee is NOT Available At This Time");
        return {
          id: -2,
        };
      } else {
